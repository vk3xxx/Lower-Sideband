...
Voice calls include outgoing route discovery, incoming ringing, answer/decline, busy and timeout handling, mute and hang-up controls, encrypted local/iCloud call history, a trusted-contacts-only policy, and native microphone/speaker routing on macOS and iOS. A bounded jitter buffer absorbs ordinary packet timing variation, reprimes cleanly after an underrun and drops stale audio before latency can grow without limit; mute continues sending encoded silence so the remote Opus stream remains stable. iOS can receive calls while its Reticulum connection is alive; reliable terminated-app or indefinitely suspended incoming calling will require a production CallKit/PushKit wake service and is not claimed by this milestone.
