...
The app starts gateway discovery and connection automatically. It first tries Bonjour-advertised `_reticulum._tcp`, `_rns._tcp`, and `_sideband._tcp` services and remembers the last successful advertised gateway. If none connect, it tries the configured private IPv6 and IPv4 endpoints. Only after local options fail does it rotate through currently published community gateways, with an optional user-supplied public IPv6 address or DNS hostname placed first. Public hostnames are resolved by Network.framework, which can use IPv6 when offered and fall back to IPv4. Network changes restart the complete local-first selection cycle. Fresh installs use `[fd20:20:20::133]:4242` and `10.20.20.133:4242` only as local fallbacks; no personal public address is embedded. Manual configuration remains available in Network Status. Native AutoInterface supports authenticated multicast peer discovery and the UDP data plane.
