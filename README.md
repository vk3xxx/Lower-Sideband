...
Voice calls include outgoing route discovery, incoming ringing, answer/decline, busy and timeout handling, mute and hang-up controls, encrypted local/iCloud call history, a trusted-contacts-only policy, and native microphone/speaker routing on macOS and iOS. iOS can receive calls while its Reticulum connection is alive; reliable terminated-app or indefinitely suspended incoming calling will require a production CallKit/PushKit wake service and is not claimed by this milestone.
