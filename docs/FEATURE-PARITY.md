...
| Nomad Network | Complete native client | Validated Nomad addresses, ReticulumKit Link requests, bounded native Micron rendering, interactive reference-compatible fields and submissions, browsing history, bookmarks, encrypted page archive and local page editing |
| Reticulum application services | Complete native workspace | Persistent RRC community state and moderation plus a validated encrypted directory for Nomad, RRC, RNSH, RNX and RNCP services with route health and recent-use status |
